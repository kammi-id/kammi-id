...
- [ ] **Ringkasan turunan.** Fungsi murni yang mengambil paragraf pertama dari
      ...
      tidak ada migrasi.
- [ ] Ringkasan dipakai untuk `description`, `openGraph.description`,
      `twitter.description`, dan `Article.description` — satu sumber, empat
      pemakai.
- [ ] Jatuhan berlapis kalau paragraf pertama tidak ada atau kosong (artikel
...
- [ ] Uji khusus untuk badan tulisan yang: kosong; dibuka gambar; dibuka
...
- [ ] `author`: `Person` dengan `name` kalau `penulis` terisi; jatuh ke
...
- [ ] `dateModified` memakai `article.updatedAt`. Kalau `updatedAt` lebih awal
...
- [ ] **Breadcrumb** dipasang di Permalink Berita: Beranda → Berita → judul.
...
- [ ] Berita ber-`noindex` (Struktur Non-Aktif, ADR 0013) **tetap** memasang
      JSON-LD. `noindex` menyangkut pengindeksan, bukan penyangkalan bahwa
      tulisannya pernah ada.
- [ ] Halaman (`type: 'page'`) ikut mendapat ringkasan turunan, canonical, dan
      breadcrumb, tapi dimarkup `WebPage`, bukan `Article` — ia tak bertanggal
      (`CONTEXT.md`).
- [ ] `check:types`, `check:lint`, `check:structure` hijau, dan uji baru lulus.
