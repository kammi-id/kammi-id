...
import { organizationView } from '~/db/schemas/views/organization.sql'
import { memberView } from '~/db/schemas/views/member.sql'
import { trainingView } from '~/db/schemas/views/training.sql'
import { managersHistoryView } from '~/db/schemas/views/manager.sql'
...
const seed = async (): Promise<void> => {
  ...
  for (const view of views) {
    try {
      ...
      console.log(`Refreshed ${view.name}`)
    } catch (error) {
      console.error(`Failed to refresh view ${view.name}:`, error)
    }
  }
